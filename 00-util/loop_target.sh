...
LOOP_TARGET() {
	...
	| while read LINE ; do
		...
		LOOP_HOSTNAME=`echo ${LINE} | cut -d ',' -f 3`
		LOOP_USERNAME=`echo ${LINE} | cut -d ',' -f 4`
		LOOP_PASSWORD=`echo ${LINE} | cut -d ',' -f 5`
		LOOP_ROOTPASS=`echo ${LINE} | cut -d ',' -f 6`

		echo
		echo ">> LOOP_TYPE : ${LOOP_TYPE}"
		echo ">> LOOP_ADDRESS : ${LOOP_ADDRESS}"
		echo ">> LOOP_HOSTNAME : ${LOOP_HOSTNAME}"
		echo ">> LOOP_USERNAME : ${LOOP_USERNAME}"
		echo ">> LOOP_PASSWORD : ${LOOP_PASSWORD}"
		echo ">> LOOP_ROOTPASS : ${LOOP_ROOTPASS}"
		echo
